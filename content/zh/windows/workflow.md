...
{{< tutorial-figure src="/tutorial-images/windows-workflow-zh.svg" caption="八阶段分析路线。步骤编号表示阅读顺序；此图为流程示意。" >}}
...
先在独立测试项目用少量样品检查参数，确认后在正式项目点击 **Process raw data** 处理完整数据。
...
**Feature filters** 改变当前显示范围，不删除下游数据；人工 **Exclude** 标记也不是自动从分析矩阵移除。详见 [结果审查](/zh/windows/feature-review/)。
...
选择运行范围和导出内容，保存完整表格、R 对象、代码和日志。详见 [导出与恢复](/zh/windows/export-and-recovery/)。
