...
| 现象 | 检查与处理 |
| --- | --- |
| 第四步无法开始 | 检查第三步是否有当前有效结果；试跑不满足正式前置条件 |
| 重跑上游后下游显示 stale | 下游仍依赖旧输入，需要从受影响步骤重新运行；历史结果保留 |
| 取消或重开后不能从内部进度继续 | 当前恢复以保存步骤为单位，不支持峰检测内部续算 |
| 试跑完成但 Raw data processing 没有打勾 | 正常；检查参数后点击 Process raw data 进行正式处理 |
...
任务中心面板可以关闭，计算会继续。若提示 R busy，说明当前操作需要新计算而 R 正忙；等待任务完成后再试。浏览已保存摘要与已缓存结果不应被理解为可以并发运行多个分析。

内存接近电脑总量时，先查看 Computer used 和 TidyMass used，区分系统其他进程与当前分析；按实际情况减少 worker 数或试跑规模。不要把增加线程数当作一定更快。
