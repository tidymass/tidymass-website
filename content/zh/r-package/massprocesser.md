...
{{< tutorial-figure src="/tutorial-images/r-package-massprocesser-zh.svg" caption="从扫描到特征强度。步骤编号表示阅读顺序；此图为流程示意。" >}}

先选择覆盖不同批次的代表性文件。ppm 依据质量精度设置，peakwidth 依据实际峰持续时间设置。处理后检查 EIC 峰形、RT 对齐和样本/特征数量；特征行数更多并不意味着参数更好。
