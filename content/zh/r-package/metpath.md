...
{{< tutorial-figure src="/tutorial-images/r-package-metpath-zh.svg" caption="从化合物 ID 到通路。步骤编号表示阅读顺序；此图为流程示意。" >}}

解释通路前先统计可映射化合物数量，对同一化合物的多个离子特征去重并保留原始映射表。结合背景定义、多重检验校正和未匹配 ID 阅读富集结果；空结果可能只是注释覆盖率有限。
